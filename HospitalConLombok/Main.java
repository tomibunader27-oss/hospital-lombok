...
public class Main {
    ...
    private static void mostrarDistribucionEspecialidades(Hospital hospital) {
        ...
        var repo = new InMemoryRepository<Paciente>();

        var p1 = Paciente.builder()
                .nombre("Pepito")
                .apellido("Honguito")
                .dni("40111222")
                .fechaNacimiento(LocalDate.of(2003, 5, 21))
                .tipoSangre(TipoSangre.O_POSITIVO)
                .telefono("261-555-0000")
                .direccion("Mendoza")
                .build();

        repo.save(p1); // => setId(1L) por reflexión

        System.out.println("ID asignado: " + p1.getId());
        repo.findAll().forEach(System.out::println);

        // Update genérico (mantiene el mismo ID)
        var pActualizado = Paciente.builder()
                .nombre("Pepito Actualizado")
                .apellido("Honguito")
                .dni("40111222")
                .fechaNacimiento(LocalDate.of(2003, 5, 21))
                .tipoSangre(TipoSangre.O_POSITIVO)
                .telefono("261-555-9999")
                .direccion("Godoy Cruz")
                .build();

        repo.genericUpdate(1L, pActualizado);
